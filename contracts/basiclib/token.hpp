...
*/
#pragma once
#include <basiclib/math.hpp>
#include <basiclib/print.hpp>
...
namespace basic {
  ...
  *  @tparam CurrencyType - type of the currency (e.g. basic) represented as an unsigned 64 bit integer
  *  @ingroup tokens
  *
  ...
  *  Example:
  *  @code
  *  typedef basic::token<uint32_t, N(MyToken)> MyToken;
  *  MyToken  a(128);
  ...
  *  @{
  */
  template<typename NumberType, uint64_t CurrencyType = N(basic) >
  struct token {
    /**
    * Type of the currency (e.g. basic) represented as an unsigned 64 bit integer
    ...
    */
    ...
    /**
    * Print as string representation of the token (e.g. 1 BASIC)
    * @brief Print as string
    */
    inline void print() {
      basic::print( quantity, " ", Name(CurrencyType) );
    }
  };
  /// @}
  ...
  *  Example:
  *  @code
  *  typedef basic::token<uint64_t, N(MyBaseToken)> MyBaseToken;
  *  typedef basic::token<uint64_t, N(MyQuoteToken)> MyQuoteToken;
  ...
  */
  ...
  struct price
  {
    ...
    */
    friend QuoteToken operator / ( BaseToken b, const price& q ) {
      basic::print( "operator/ ", uint128(b.quantity), " * ", uint128( precision ), " / ", q.base_per_quote, "\n" );
    ...
    */
    friend BaseToken operator * ( const QuoteToken& b, const price& q ) {
      basic::print( "b: ", b, " \n" );
      basic::print( "operator* ", uint128(b.quantity), " * ", uint128( q.base_per_quote ), " / ", precision, "\n" );
    ...
    */
    inline void print() {
      basic::print( base_per_quote, ".", " ", Name(base_token_type::currency_type), "/", Name(quote_token_type::currency_type)  );
    }
  private:
    /**
  ...
  /**
  * @typedef Tokens
  * @brief Defines basic tokens
  * @details Defines basic tokens
  */
  typedef basic::token<uint64_t,N(basic)>   Tokens;

  /**
  *  @struct basic::Transfer
  *  @brief The binary structure of the `transfer` message type for the `basic` contract.
  *  @ingroup tokens
  *
  *  @details
  *  Example:
