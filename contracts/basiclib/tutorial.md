# Developing Smart Contracts / Decentralized Applications with BASIC-CHAIN

This tutorial is targeted toward those who would like to get started building decentralized applications or
smart contracts using the BASIC-CHAIN software development kit. It will cover writing contracts/applications in C++
...
This tutorial assumes that you have basic understanding of how to use 'basicd' and 'basicc' to setup a node
...
BASIC-CHAIN based blockchains execute user generated applications and code using Web Assembly (WASM). WASM is an
...
The BASIC-CHAIN software only officially supports unix environments and building with clang.
...
To keep things simple we have created a tool called `basiccpp` which can be used to bootstrap a new contract. For this
to work we assume you have installed the basicio/basic code installed and that ${CMAKE_INSTALL_PREFIX}/bin is in your path.
...
```bash
$ basiccpp -o hello.wast hello.cpp
```
...
1. start basicd with wallet plugin enabled
...
```bash
$ basicc set contract ${account} hello.wast hello.abi
Reading WAST...
Assembling WASM...
Publishing contract...
{
...
If you are monitoring the output of your basicd process you should see:
...
You will notice the lines "Init World!" are executed 3 times, this isn't a mistake.  When the blockchain is processing transactions the following happens:
   ...
   1. basicd receives a new transaction
   ...
   2. basicd starts to produce a block
   ...
   3. basicd pushes the generated block as if it received it from the network
...
```bash
$ basicc push message ${account} hello '"abcd"' --scope ${account}
```
...
If you are following along in `basicd` then you should have seen the following scroll by the screen:
...
You will notice that this ABI defines an action `transfer` of type `transfer`.  This tells BASIC-CHAIN that when `${account}->transfer` message is seen that the payload is of type
...
```bash
basicc push message ${account} transfer '{"from":"currency","to":"inita","amount":50}' --scope initc
...
If you observe the output of `basicd` you should see:
...
The BASIC-CHAIN C API provides access to the message payload via the Message API:
...
extern "C" {
    ...
    void apply( uint64_t code, uint64_t action ) {
       ...
       if( action == N(transfer) ) {
          ...
          basic::print( "Transfer ", message.amount, " from ", basic::Name(message.from), " to ", basic::Name(message.to), "\n" );
       }
    }
...
`basicd` will call init() again because of the redeploy
...
```bash
$ basicc push message ${account} transfer '{"from":"currency","to":"inita","amount":50}' --scope ${account}
{
...
And on `basicd` we should see the following output:
...
So far we used the C API because it is the lowest level API that is directly exposed by BASIC-CHAIN to the WASM virtual machine. Fortunately, basiclib provides a higher level API that
removes much of the boiler plate. 

```c
/// basiclib/message.hpp
namespace basic {
   template<typename T>
   T currentMessage();
}
...
extern "C" {
    ...
    void apply( uint64_t code, uint64_t action ) {
       basic::print( "Hello World: ", basic::Name(code), "->", basic::Name(action), "\n" );
       if( action == N(transfer) ) {
          auto message = basic::currentMessage<transfer>();
          basic::print( "Transfer ", message.amount, " from ", message.from, " to ", message.to, "\n" );
       }
    }
...
You will notice that we updated the `transfer` struct to use the `basic::Name` type directly, and then condenced the checks around `readMessage` to a single call to `currentMessage`.
...
The BASIC-CHAIN software will take care of enforcing and validating the signatures, all you need to do is require the necessary authority.
...
```c
    ...
    void apply( uint64_t code, uint64_t action ) {
       ...
       if( action == N(transfer) ) {
          auto message = basic::currentMessage<transfer>();
          basic::requireAuth( message.from );
          basic::print( "Transfer ", message.amount, " from ", message.from, " to ", message.to, "\n" );
       }
    }
    ...
...
```bash
 basicc push message ${account} transfer '{"from":"initb","to":"inita","amount":50}' --scope ${account}
...
If you look on `basicd` you will see this:
```
...
This shows that it attempted to apply your transaction, printed the initial "Hello World" and then aborted when `basic::requireAuth` failed to find authorization of account `initb`.

We can fix that by telling `basicc` to add the required permission:

```bash
 basicc push message ${account} transfer '{"from":"initb","to":"inita","amount":50}' --scope ${account} --permission initb@active
```
...
```bash
 $ basiccpp -o hello.wast hello.cpp
 $ basicc set contract ${account} hello.wast hello.abi
 $ basicc push message ${account} transfer '{"from":"initb","to":"inita","amount":0}' --scope initc --permission initb@active
