#include <basiclib/types.hpp>
#include <basiclib/message.hpp>
#include <basiclib/db.h>
...
unsigned int test_db::key_i64i64i64_general() {
  ...
  #define TABLE1_ASSERT(I, V, msg) \
    if(LOGME) {\
      basic::print(msg, " : ", res, " a:", V.a, " b:", V.b, " c:", V.c, " t:", V.table, " ("); \
      basic::print(BS(res == sizeof(V)), " ", BS(records[I].a == V.a), " ", BS(records[I].b == V.b), " ", BS(records[I].c == V.c), " => ", N(table1), ")\n"); \
    } \
  ...
  #define LOAD_OK(I, O, T, INX, MSG) \
    {basic::remove_reference<decltype(V)>::type tmp; my_memset(&tmp, 0, sizeof(tmp));tmp = V; \
    res = LOAD(I, O, T, tmp); \
  ...
  { \
    ...
    auto j = 0; \
    do { \
      basic::remove_reference<decltype(records[0])>::type tmp = records[I[j]]; \
  ...
  { \
    ...
    auto j = 0; \
    do { \
      basic::remove_reference<decltype(records[0])>::type tmp = records[j]; \
...
//basic::print("xxxx ", res, " ", tmp2.name, " ", uint64_t(tmp2.age), " ", tmp2.phone, " ", tmp2.new_field, "\n");
