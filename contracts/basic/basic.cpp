...
namespace native {
  /**
    @defgroup BASICcontract BASIC Contract
    @brief Documents the interface to the BASIC currency contract
    @ingroup contracts
    @{
  */
   ...
   /**
    *   @class BASIC
    *   @brief A *native* currency contract implemented with account named `BASIC`
    *   @ingroup contracts
    *
    *   @details The BASIC contract is a *native* currency contract implemented with account named `BASIC`. This contract enables
    *   users to transfer BASIC tokens to each other. This contract is designed to work the @ref stakedcontract and
    *   @ref systemcontract when creating new accounts, claiming staked BASIC.
    */
   struct BASIC : public contract {
      ...
      access to the `BASIC` scope which means that multiple transfers can execute in parallel
      ...
      other contracts are notified anytime BASIC tokens are transferred.

      */
      void transfer (
         AccountName from,  ///< account from which BASIC will be withdrawn
         AccountName to,    ///< account to receive BASIC, may not be same as `from`
